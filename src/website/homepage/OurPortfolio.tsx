          ...
          <div className=" overflow-x-auto bars pt-8 lg:pt-[4rem]">
            <div className="lg:grid flex lg:grid-cols-3 gap-4">
              {data.length > 3
                ? data
                    .slice(0, 3)
                    .map((item, idx) => <Card item={item} key={idx} />)
...
function Card({ item }: { item: any }) {
  return (
    <Link href={`/portfolio/${item.id}`}>
      <article className="grid w-[18rem] lg:w-full gap-2">
        ...
        <div className="pt-1 grid gap-1">
          <h1 className="text-2xl font-medium">{item.title.rendered}</h1>
          <p className="text-lg line-clamp-2">{item.acf.text}</p>
        </div>
      </article>
    </Link>
  );
